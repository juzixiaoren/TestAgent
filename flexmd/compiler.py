...
A deliberately small, testable Markdown-presentation language:
- root-level `---` separates slides;
- repeated root-level `---` creates empty slides;
- inside `\\h ... \\\\h`, `---` separates horizontal panes;
- inside `\\v ... \\\\v`, `---` separates vertical panes;
- repeated pane separators create empty panes;
- flex start markers may carry ratios, for example `\\h 1:2`;
- fenced code blocks protect all special markers inside them.

This module is both a usable compiler and the target program tested by the
software testing agent in this lab.
"""
...
def iter_lines(source: str) -> list[tuple[int, str]]:
    # split("\n") keeps trailing empty lines observable and normalizes Windows line endings.
...
def parse_open_flex(line: str, line_no: int) -> Optional[tuple[str, Optional[list[int | float]]]]:
    """Parse a flex opener such as `\\h`, `\\v`, `\\h 1:2` or `\\v 2:1:1`."""
    stripped = line.strip()
...
class Parser:
    ...
    def _flush_markdown(self) -> None:
        ...
        # Empty panes and empty slides are represented by empty child lists.
        # Therefore, only non-empty markdown creates a MarkdownNode.
        if content.strip():
            ctx.container.append(MarkdownNode(content=content))

    def _start_new_slide(self) -> None:
        self._flush_markdown()
        # Repeated root-level separators are meaningful: they create empty slides.
    ...
    def _start_new_pane(self, line_no: int) -> None:
        ...
        # Repeated separators are meaningful: they create empty panes.
        pane = PaneNode()
    ...
    def _consume_line(self, line_no: int, line: str) -> None:
        # Intentional mutant for experiment report: code fences are not respected,
        # so `---` inside code blocks is incorrectly parsed.
